...
interface PrinterDevice {
    name: string;
    target: string; // MAC Address or UUID
    [key: string]: any;
}
...
class PrinterService {
  connectedPrinter: PrinterDevice | null = null;
  printerWidth: number = 48; // 80mm = 48 caracteres, 58mm = 32 caracteres
   ...
   */
  async printPedido(pedidoData: PedidoData): Promise<boolean> {
    if (!this.connectedPrinter) {
      Alert.alert('Sem impressora', 'Conecte-se a uma impressora primeiro.');
      return false;
    }
