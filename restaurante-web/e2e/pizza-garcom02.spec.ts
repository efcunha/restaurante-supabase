...
test('Pedido de Pizza - Garcom 02', async ({ page }) => {
  test.setTimeout(180000); // Aumentado para 3 minutos

  // 1. Login
  console.log('Realizando login com Garcom 02...');
  await page.goto('/');
  await page.getByPlaceholder('seu@email.com').fill(process.env.PLAYWRIGHT_TEST_EMAIL_GARCOM02!);
  await page.getByPlaceholder('••••••••').fill(process.env.PLAYWRIGHT_TEST_PASSWORD_GARCOM02!);
  ...
  // Adiciona um pequeno delay aleatório para evitar race condition
  ...
  await page.getByPlaceholder('Digite o nome').fill(`PEDIDO PIZZA G02 ${uniqueId}`);
  ...
  // 9. Adicionar itens extras padronizados
  ...
  console.log('- Pedido Pizza criado por Garcom 02!');
});
