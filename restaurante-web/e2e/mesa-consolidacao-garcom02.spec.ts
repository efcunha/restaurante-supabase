...
async function cancelOpenComandasByMesa(mesa: string, accessToken: string) {
  await fetch(`${SUPABASE_URL}/rest/v1/comandas?status=eq.aberta&date_key=eq.${TODAY_KEY}&or=(table_number.eq.${mesa},mesa.eq.${mesa})`, {
    ...
    body: JSON.stringify({
      ...
      motivo_cancelamento: 'cleanup-mesa-consolidacao-e2e-garcom02',
    }),
  });
}
...
test.describe('Consolidacao Mesa -> Comanda Canônica - Garcom 02', () => {
  test.setTimeout(180000);

  test('Consolidar mesa origem em destino deve manter 1 comanda aberta operacional na mesa destino - Garcom 02', async ({ page }) => {
    ...
    try {
      ...
      const markMergedRes = await fetch(
        `${SUPABASE_URL}/rest/v1/comandas?table_number=eq.${sourceMesa}&comanda_number=eq.${sourceComanda}&status=eq.aberta&date_key=eq.${TODAY_KEY}`,
        {
          ...
          body: JSON.stringify({
            ...
            merged_by: userId,
            merge_reason: 'e2e-consolidacao-mesa-garcom02',
          }),
        }
      );
      ...
      console.log('✓ Teste de consolidação por Garcom 02 passou com sucesso!');
    } finally {
