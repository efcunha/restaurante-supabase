...
class OrderFirestoreService {
  ...
  private mapRowToOrder(row: any): Order {
    return {
      ...
      orderOrigin: row.order_origin || 'standard',
      operationalRoute: row.operational_route || 'production',
      servicePoint: row.service_point || null,
      autoGeneratedComanda: row.auto_generated_comanda || false,
    } as Order;
  }
  ...
  private async _createOrderInternal(companyId: string, order: Partial<Order>): Promise<string> {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        order_origin: order.orderOrigin || 'standard',
        operational_route: order.operationalRoute || 'production',
        service_point: order.servicePoint || null,
        auto_generated_comanda: order.autoGeneratedComanda || false,
  ...
  private _mapUpdatesToPayload(updates: Partial<Order>) {
    ...
    if (updates.orderOrigin) payload.order_origin = updates.orderOrigin;
    if (updates.operationalRoute) payload.operational_route = updates.operationalRoute;
    if (updates.servicePoint !== undefined) payload.service_point = updates.servicePoint;
    if (updates.autoGeneratedComanda !== undefined) payload.auto_generated_comanda = updates.autoGeneratedComanda;
