/**
 * ObservabilityService.ts — restaurante-web
 *
 ...
 * Deve ser chamado UMA VEZ na inicializacao do app (ex: App.js).
...
export function installGlobalErrorHandler(): void {
  ...
  // ErrorUtils não está disponível em ambiente web (browser) — apenas no runtime React Native.
  ...
  ErrorUtils.setGlobalHandler((error: Error, isFatal?: boolean) => {
    ...
    // Delegar para Sentry e handler anterior
...
/** Navegacao de pagina (React Navigation) */
export function logPageView(routeName: string, userId?: string): void {
  sendLogToOps({
    level: 'info',
    event: 'page_view',
    message: `Navegou para ${routeName}`,
    user_id: userId,
    metadata: { route: routeName, source: SERVICE_NAME },
  });
}
