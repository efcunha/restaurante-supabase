...
import { Order, OrderItemStatus, SelfServiceFlowContext } from '../types';
...
class OrderService {
  ...
  generateItemsWithStatus(
    ...
    expandedItems.forEach((info) => {
      ...
      const itemMetadata = itemMetadataMap[info.itemNameWithoutQty.toLowerCase()] || itemMetadataMap[lowerName];

      for (let i = 0; i < info.qty; i++) {
        const itemWithStatus: any = {
        ...
        if (itemMetadata?.measurementType === 'weight') {
          itemWithStatus.measurementType = 'weight';
          itemWithStatus.weightKg = Number(itemMetadata.weightKg) || 0;
          itemWithStatus.weightSource = itemMetadata.weightSource === 'manual' ? 'manual' : 'automatic';
          itemWithStatus.weightUnit = 'kg';
        }

        result.push(itemWithStatus as OrderItemStatus);
        absoluteIndex++;
      }
    });
  ...
  createOrder(
    ...
    const itemsWithStatus = this.generateItemsWithStatus(items, orderId, comanda, categoryMap, priceMap, itemMetadataMap);
    ...
    const order: Order = {
      ...
      createdBy,
      createdByName,
      orderOrigin: flowContext?.orderOrigin || 'standard',
      operationalRoute: flowContext?.operationalRoute || 'production',
      servicePoint: flowContext?.servicePoint || null,
      autoGeneratedComanda: flowContext?.autoGeneratedComanda || false,
  ...
  * Regra: só pode cancelar se o item ainda não foi entregue
  ...
  shouldBypassOperationalQueues(order: Pick<Order, 'orderOrigin' | 'operationalRoute'> | null | undefined): boolean {
    if (!order) return false;

    return order.operationalRoute === 'bypass_production'
      || order.orderOrigin === 'self_service_scale';
  }
  ...
  filterOrdersByStatus(orders: Order[], status: string): Order[] {
    const visibleOrders = orders.filter(order => !this.shouldBypassOperationalQueues(order));

    if (status === 'churrasqueira') {
      // Churrasqueira enxerga pedidos em montagem (visualização, sem interação)
      return visibleOrders.filter(order => order.status === 'preparing');
    }
    if (status === 'montagem') {
