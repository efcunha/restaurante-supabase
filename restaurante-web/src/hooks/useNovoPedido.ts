...
export function useNovoPedido(): UseNovoPedidoReturn {
    ...
    const applyCachedCardapio = useCallback((parsedCache: { data: Cardapio; pizzaConfig?: PizzaConfig | null; pizzaSubcategories?: string[]; bebidaSubcategories?: string[]; extras?: any[] }) => {
    ...
    const carregarCardapioSupabase = useCallback(async ({ showGlobalLoading = true }: { showGlobalLoading?: boolean } = {}) => {
        try {
            ...
            const resolvedPizzaSubcategories = resolveConfiguredSubcategories(
                companySettings?.pizzaSubcategories,
                []
            );
            ...
            await AsyncStorage.setItem(MENU_STORAGE_NAMESPACE_V2, JSON.stringify({
                ...
                bebidaSubcategories: resolvedBebidaSubcategories,
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (user?.companyId) {
                ...
                if (!caixaAberto) {
                    const caixaMensagem = 'O caixa precisa estar aberto para criar pedidos. Vá em "Caixa" > "Abertura" para abrir o caixa do dia.';
                    showToast(caixaMensagem, 'warning');
                    Alert.alert(
                        '⚠️ Caixa Fechado',
            ...
            if (error?.code === 'CAIXA_FECHADO') {
                ...
                // Em web, o toast é mais confiável visualmente que Alert.alert em alguns navegadores.
                showToast(caixaMensagem, 'warning');
