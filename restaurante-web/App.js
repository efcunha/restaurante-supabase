...
import { SIDEBAR_WIDTH, WebSidebarTabBar } from './src/components/WebSidebarTabBar'
    ...
    return (
      <SafeAreaProvider>
        ...
        <NavigationContainer
          ...
          documentTitle={{
            formatter: (opts) =>
              opts?.route?.name === 'PublicMenu' ? 'Cardápio' : 'Restaurante Web',
          }}
          linking={{
            prefixes: ['/'],
            config: {
        ...
        >
          <RootNavigator>{() => <AuthStack />}</RootNavigator>
        </NavigationContainer>
      </SafeAreaProvider>
    )
  }
