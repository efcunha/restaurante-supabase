...
export function Hero() {
  ...
  const [androidQrValue, setAndroidQrValue] = useState(androidHref)

  useEffect(() => {
    if (typeof window === 'undefined') return

    try {
      setAndroidQrValue(new URL(androidHref, window.location.origin).toString())
    } catch {
      setAndroidQrValue(androidHref)
    }
  }, [androidHref])
      ...
      <Container className="text-center">
        ...
        >
          ...
          <div className="bg-surface border border-border rounded-xl p-6 text-left hover:border-strong transition-colors duration-base group flex flex-col">
            ...
            <p className="text-sm text-foreground-muted mb-3">React Native — balcão, mesa, delivery, comandas</p>
          ...
          <div className="bg-surface border border-border rounded-xl p-6 text-left hover:border-strong transition-colors duration-base group flex flex-col">
            ...
            <p className="text-sm text-foreground-muted mb-3">Expo Web — gestão e espelho das operações</p>
