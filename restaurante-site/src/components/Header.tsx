        ...
        <nav className="hidden md:flex items-center gap-1">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="px-3 py-2 text-sm text-foreground-muted hover:text-foreground transition-colors duration-base rounded-md hover:bg-accent-subtle"
            >
              {link.label}
            </a>
          ))}
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="md:hidden fixed inset-0 top-16 bg-background/95 backdrop-blur-lg z-sticky px-6 py-8"
          >
