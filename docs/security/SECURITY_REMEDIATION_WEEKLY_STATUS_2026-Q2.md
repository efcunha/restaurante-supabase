...
| `CURSOR_SECRET` sem hardcode | app + web | em andamento | `docs/security/SEC-W1-002-CURSOR-SECRET-DEPLOYMENT.md`, `restaurante-app/src/utils/cursorValidation.ts`, `restaurante-web/src/utils/cursorValidation.ts`, `.env.example` em ambos | Secret gerado: `9f05e59a28393b3c7684abf8f98d9d226a1a47ea1383ba7c74b04be89d868fd6` (01/04, 16:15 UTC). Aplicacao confirmada em `restaurante-web` (Railway) em 01/04. Build app Android validado novamente em 01/04 via `./scripts/build-android.sh` (APKs `1635_*` gerados), confirmando carregamento local de `CURSOR_SECRET`. Logs web (Railway) com container aceitando conexoes e respostas `200/304`, sem erro `CURSOR_SECRET not configured`. Runtime AppEntry mostrou erro inicial de refresh token invalido seguido de recuperacao (`SIGNED_IN`) e operacao normal de telas/dados. Pendente: validacao explicita de paginacao (app/web) para marcar concluido. |
