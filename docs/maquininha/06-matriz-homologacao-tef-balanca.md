...
| TEF-08 | MOCK_AUTO | Endpoint indisponivel na iniciacao | Flags PDV habilitadas | Mock de `POST /payments/initiate` falhando | Executar inicio da maquininha | Frontend mostra erro operacional sem travar fluxo | Novo teste ou evidencia manual controlada | Pendente |
| TEF-09 | MOCK_AUTO | `EXPO_PUBLIC_OPS_BASE_URL` ausente | Flag PDV habilitada e env ausente | Valor valido | Acionar pagamento por maquininha | Servico retorna erro de configuracao sem chamar rede | Teste unitario ou evidencia de service call | Pendente |
| TEF-10 | MOCK_AUTO | Feature flag desabilitada | `pdv_enabled=false` ou `pdv_devicePayment_enabled=false` | Valor valido | Acionar fluxo de maquininha | Frontend bloqueia fluxo com mensagem coerente | Teste automatizado ou evidência controlada | Pendente |
| TEF-11 | INT_REAL | Iniciar transacao real controlada | Flags PDV habilitadas, usuario autenticado, OPS acessivel | `companyId`, comanda, valor e metodo validos | Iniciar maquininha a partir da UI | Backend aceita request e retorna `processing` ou estado equivalente | Log sanitizado, resposta do endpoint e evidencia visual da UI | Pendente |
| TEF-12 | INT_REAL | Confirmar transacao aprovada | Mesmo contexto do item anterior | `transactionId` real | Aguardar polling ate finalizacao | Estado final `approved` sem duplicidade de baixa | Evidencia de UI, resposta do status e persistencia coerente da comanda | Pendente |
| TEF-13 | INT_REAL | Confirmar timeout operacional | Mesmo contexto do item anterior | `transactionId` com atraso/nao conclusao | Deixar polling expirar | Frontend mostra timeout e nao registra sucesso falso | Evidencia visual e log sanitizado do status final | Pendente |
| TEF-14 | INT_REAL | Retry sem duplicidade de sucesso | Transacao anterior falhou/expirou | Nova tentativa para mesma comanda | Repetir acao controladamente | Nao ocorre duplicidade de quitacao ou sucesso fantasma | Evidencia de comanda, logs e trilha de status | Pendente |
| TEF-15 | INT_REAL | Validar bloqueio por saldo/comanda invalida | Comanda sem saldo ou invalida | Entrada incoerente | Acionar fluxo TEF | UI e/ou backend impedem processamento | Evidencia de mensagem operacional e ausencia de registro indevido | Pendente |
...
| BAL-06 | MOCK_AUTO | Leitura instavel | Flags PDV e balanca habilitadas | Mock de fetch com `estavel=false` | Executar leitura | Resultado `unstable` sem confirmar peso | Novo teste automatizado ou evidencia controlada | Pendente |
| BAL-07 | MOCK_AUTO | Erro inesperado do bridge | Flags PDV e balanca habilitadas | Mock de fetch rejeitando erro generico | Executar leitura | Resultado `error` com mensagem operacional | Novo teste automatizado | Pendente |
| BAL-08 | MOCK_AUTO | Feature flag da balanca desligada | `pdv_enabled=false` ou `pdv_scale_enabled=false` | Chamada ao service | Executar leitura | Service retorna erro por flag sem chamar bridge | Teste automatizado ou evidencia de chamada | Pendente |
