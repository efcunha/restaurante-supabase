...
// Railway ingress reverse proxy, so plain HTTP inside the container is expected.
...
const server = http.createServer((req, res) => {
  requestHandler(req, res).catch((error) => {
    console.error('[restaurante-web-storybook] request failed', error);
...
server.listen(port, () => {
  console.log(`[restaurante-web-storybook] listening on http://127.0.0.1:${port}`);
  console.log(`[restaurante-web-storybook] distDir=${distDir}`);
  console.log(`[restaurante-web-storybook] basicAuthConfigured=${basicUser && basicPass ? 'yes' : 'no'}`);
});